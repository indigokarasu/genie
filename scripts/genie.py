...
def discover_filesystem():
    ...
    # Check for large directories at ~/ that aren't in known zones
            ...
            for entry in os.listdir("/root"):
                ...
                if size > 100 * 1024 * 1024:  # > 100MB
                    discovered[f"root-{entry}"] = {
                        "tier": 3, "action": "analyze_only",
                        "path": full,
                        "description": f"Unknown ~/ directory — {fmt(size)} (review manually)",
...
def assess(cfg, targets=None):
    ...
    # Git clones
    gcp = cfg.get("git_clones_path", "~/projects")
...
def clean(cfg):
    ...
    git_clones_path = cfg.get("git_clones_path", "~/projects")
