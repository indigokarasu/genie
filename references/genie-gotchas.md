...
- **`~` path resolution in cron context**: When running as a cron job, `HOME` is set to the profile-scoped path, not `~/`. **Always use absolute paths** (`<hermes-home>/...`) in cron context.
