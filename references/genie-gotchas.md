...
- **Built-in snapshot path default is wrong for profile-scoped setups**: The default `snapshots_path` is `<hermes-home>/state-snapshots`, but in profile-scoped setups snapshots live at `<hermes-home>/profiles/indigo/state-snapshots`. Configurable via `skills.config.genie.snapshots_path` in config.yaml (legacy `GENIE_*` env vars migrated to `skills.config.genie.*`). The FILESYSTEM.md manifest corrects this, but if the manifest is missing or stale, genie won't find snapshots.
