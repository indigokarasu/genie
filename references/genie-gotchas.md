...
- **Off-hermes .cache consumers**: When `~/.cache` is large, the usual culprits are: `ms-playwright/` (browser binaries, 1G+ rebuildable), `puccinialin/` (cargo/rustup cache, rebuildable), `chroma/` (embedding DB — check before deleting), `camoufox/` (stealth browser profile cache, 1-2GB, rebuildable). These are all Tier 1 safe rebuildable caches.
...
- **Built-in snapshot path default is wrong for profile-scoped setups**: The default `GENIE_SNAPSHOTS_PATH` is `<hermes-root>/state-snapshots`, but in profile-scoped setups snapshots live at `<hermes-home>/state-snapshots`. The FILESYSTEM.md manifest corrects this, but if the manifest is missing or stale, genie won't find snapshots.
...
- **state.db is often a symlink**: `<hermes-root>/state.db` may be a symlink to a profile-scoped DB (e.g., `→ <hermes-home>/state.db`). Always resolve with `readlink -f` before operating on it. Multiple "instances" found by `find` may actually be the same file reached via symlinks + hardlinks. Check inode: `ls -li` to deduplicate.
...
- **`~` path resolution in cron context**: When running as a cron job, `HOME` is set to the profile-scoped path, not `~/`. **Always use absolute paths** (`<hermes-root>/...`) in cron context.
