...
Then a one-time destructive prune of 4 of 5 `<fs-root>/backup` dirs (~12GB) frees the danger zone.

## Symptom -> triage map
- `df -h /` >= 80% AND `<fs-root>/backup` has multiple `active-dbs-*` dirs from the same day -> this trap.
