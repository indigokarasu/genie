...
- Prune (`find ~/backup -mtime +3`) moved to ~line 18, **before** the 14G `cp <hermes-home>/state.db` at ~line 67.
...
Earlier scans concluded "backup scripts have ZERO retention logic" — WRONG. They inspected `backup_system.sh` (`<hermes-home>/profiles/indigo/scripts/backup_system.sh` and the ocas-custodian copy), which targets `~/backups` (plural) and is **UNUSED**. The live writer is `backup_all_hermes_data.sh` (singular `~/backup`).
