# Backup Prune-Order / Disk-Full Trap (CONFIRMED root cause of ~/backup bloat)

## STATUS: FIX APPLIED (2026-07-14)
The three root-cause guards are now present in the canonical live writer `<backup-repo>/scripts/backup_all_hermes_data.sh` (applied by finch:work, verified 2026-07-14T12:33). The bug is dormant and should not refire.
- Prune (`find ~/backup -mtime +3`) moved to ~line 18, **before** the 14G `cp <hermes-root>/state.db` at ~line 67.
...
The one-time destructive prune (4 of 5 `~/backup` dirs) was also completed in an earlier run; `~/backup` now holds 1 dir (3.2G). The separate TASK-015 (state.db FTS-trigram bloat, VACUUM/retention) remains open and is NOT addressed by this fix.

## Verification recipe (confirm guards are present — do NOT re-patch blindly)
```bash
SCRIPT=<backup-repo>/scripts/backup_all_hermes_data.sh
...
- Canonical: `<backup-repo>/scripts/backup_all_hermes_data.sh`
- Wrappers that exec it:
  - `<hermes-home>/skills/ocas-custodian/scripts/backup_all_hermes_data.sh`
  - `<hermes-home>/scripts/backup_all_hermes_data.sh` (3-line)

Cron job: **"Backup Hermes Sessions to GitHub"**, schedule `0 */6 * * *` (no retry config).

Key structure (canonical script):
- `set -euo pipefail`
- `BACKUP_DIR="~/backup/$TIMESTAMP"` (line 9), `mkdir -p` (line 12)
- Copies small DBs (chronicle.lbug etc.)
- `cp <hermes-root>/state.db "$BACKUP_DIR/state.db"` at **line 51** — `state.db` is a **symlink -> `<hermes-home>/state.db`, ~14G**
- GitHub push (lines ~80-112)
- Prune at **line 114**: `find ~/backup -maxdepth 1 -mindepth 1 -type d -mtime +3 -exec rm -rf {} + 2>/dev/null || true`
...
All `~/backup/active-dbs-*` dirs contain only `chronicle.db` and **NO `state.db`**. That missing 14G file is the proof of ENOSPC abort — the small copies succeeded, the large one failed, script died before prune. In the observed case: 5 dirs (~15GB), 3 created within 4 minutes (19:19 / 19:21 / 19:23) = failed cron refires.

## Decoy-script trap (do NOT repeat this mistake)
Earlier scans concluded "backup scripts have ZERO retention logic" — WRONG. They inspected `backup_system.sh` (`<hermes-home>/scripts/backup_system.sh` and the ocas-custodian copy), which targets `~/backups` (plural) and is **UNUSED**. The live writer is `backup_all_hermes_data.sh` (singular `~/backup`).

**Re-investigation rule:** to find the live writer, follow the real chain — read `jobs.json` `command`/`script_path`, grep `<hermes-root>/cron/output/<id>/*.md` logs for the job name, or follow `exec` chains in wrappers. Do NOT conclude root cause from a plausibly-named sibling script. Also confirm which directory actually receives writes (`~/backup` here, not `~/backups`). And check whether a retention line is *reachable* — a prune after a large/failing `cp` under `set -e` is effectively dead code on a full disk.
...
Then a one-time destructive prune of 4 of 5 `~/backup` dirs (~12GB) frees the danger zone.

## Symptom -> triage map
- `df -h /` >= 80% AND `~/backup` has multiple `active-dbs-*` dirs from the same day -> this trap.
