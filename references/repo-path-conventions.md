...
2. If yes → confirm remotes match, then delete the `~/` copy
3. If no → `git clone <url> projects/github-staging<reponame> && rm -rf ~/<reponame>`
...
- `indigo-repo`, `BOOK`, `get-md-work` found in `~/` → deleted (copies already in `github-staging/`)
