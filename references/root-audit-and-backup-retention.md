...
```bash
du -xhd2 <hermes-root> ~/projects ~/backup ~/backups /var 2>/dev/null | sort -hr | head -100
```
...
```bash
find ~/backup ~/backups <hermes-root>/state-snapshots <hermes-root>/migrations -xdev -mindepth 1 -maxdepth 4 \
...
```bash
du -sh ~/*/ ~/projects/*/ <staging-dir>/*/ 2>/dev/null | sort -hr | head -80
```
...
grep '~/' <hermes-root>/config.yaml <hermes-root>/profiles/*/config.yaml 2>/dev/null || true
```
