...
```bash
du -xhd2 <hermes-home> ~/projects ~/backup ~/backups /var 2>/dev/null | sort -hr | head -100
```
...
```bash
find ~/backup ~/backups <hermes-home>/state-snapshots <hermes-home>/migrations -xdev -mindepth 1 -maxdepth 4 \
...
```bash
du -sh ~/*/ <projects-root>/*/ <projects-root>/github-staging/*/ 2>/dev/null | sort -hr | head -80
```
...
```bash
grep -rl '~/' /etc/systemd/system/ 2>/dev/null | grep -v hermes || true
ps aux | grep '~/' | grep -v grep | grep -v '.hermes' || true
grep -n '~/' ~/.bashrc ~/.profile ~/.zshrc 2>/dev/null | grep -v '#' || true
crontab -l 2>/dev/null || true
hermes cron list 2>/dev/null || true
grep '~/' <hermes-home>/config.yaml <hermes-home>/profiles/*/config.yaml 2>/dev/null || true
```
