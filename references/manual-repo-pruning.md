...
| Blocker | Symptom | Fix |
|---|---|---|
| Dead local `origin` | `origin` = `~/Graze` (not a repo); push → "Could not read from remote" | The real GitHub upstream is often present under a misnamed remote (`github`, `nithub`). `git push <real-remote> <branch>` instead of `origin`. |
