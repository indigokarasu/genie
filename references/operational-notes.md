...
2. Manual: removed `~/.cache/camoufox/` (1.4 GB — not covered by genie)
...
### Key Learnings
1. **camoufox cache is a new large consumer**: stealth browser profile cache at `~/.cache/camoufox/` was 1.4 GB. Not covered by genie's built-in cache rules. Safe to `rm -rf` (rebuildable on next browser use).
...
### Remaining Concerns
- `~/backup/`: 9.4 GB single directory from June 2026-06-24 (chroma.sqlite3 + chronicle.lbug) — manual review needed
...
6. **braun-build-sources is a build cache, not a backup**: Contains cloned GitHub repos used during skill installation. Safe to delete once skills are installed. Check `du -sh ~/backups/braun-*` to identify.
