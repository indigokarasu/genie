...
**IMPORTANT:** Always use absolute paths (`/root/...`), never `~`. In cron context, `~` resolves to the profile-scoped home (`<hermes-home>/home/`), not `/root/`.
...
- **`~` path resolution in cron context**: When running as a cron job, `HOME` is set to the profile-scoped path (`<hermes-home>/home/`), not `/root/`. The `read_file` tool expands `~` to this profile home, causing "file not found" errors for paths like `~/.hermes/scripts/genie.py`. **Always use absolute paths** (`<hermes-root>/...`) in cron context. The `terminal` tool does not have this issue — it resolves `~` from the actual user home.
