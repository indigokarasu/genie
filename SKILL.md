...
1. **Manual backups** (`~/backup/`, `~/backups/`) — enforce one historical backup total on the VPS. Keep newest valid backup; older backups are reclaimable.
...
- **Do not preserve multiple backups**: The VPS policy is one historical backup plus current live data. Count all historical backup classes together (`~/backup`, `~/backups`, snapshots, migration backups, `.bak-*`) and report/remove older valid copies.
...
### Decoy-script trap + unreachable prune (CONFIRMED root cause of ~/backup bloat, 2026-07-13)
When re-investigating disk spikes from backup copies, the **live writer is `backup_all_hermes_data.sh`** (`<backup-repo>/scripts/...`, exec'd via the ocas-custodian wrapper), NOT `backup_system.sh`. `backup_system.sh` targets `~/backups` (plural) and is **unused** — an earlier scan blamed it for "zero retention logic," which was wrong. The live `backup_all_hermes_data.sh` DOES have a prune line (`find ~/backup -mtime +3 -exec rm -rf`), but it sits at **line 114**, *after* the `cp <hermes-home>/state.db` (symlink -> 14G profile DB) at **line 51**, under `set -euo pipefail`. On a near-full disk the `cp` hits ENOSPC, the script aborts, and the prune **never runs** -> partial `active-dbs-*` dirs accumulate (all lacking `state.db` = the ENOSPC-abort signature).
...
| `references/backup-prune-diskfull-trap.md` | CONFIRMED root cause of ~/backup bloat: live writer is backup_all_hermes_data.sh; prune line unreachable (after 14G state.db cp under set -e); decoy-script trap. **FIX APPLIED 2026-07-14 — top of file has STATUS + a Verification recipe; run it before re-patching.** |
