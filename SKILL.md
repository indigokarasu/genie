...
| `GENIE_STATE_DB_PATH` | `<hermes-root>/state.db` | Path to state.db |
| `GENIE_SESSIONS_PATH` | `<hermes-root>/sessions` | Path to session files |
| `GENIE_LOGS_PATH` | `<hermes-root>/logs` | Path to log files |
| `GENIE_CRON_OUTPUT_PATH` | `<hermes-root>/cron/output` | Path to cron output |
| `GENIE_SNAPSHOTS_PATH` | `<hermes-root>/state-snapshots` | Path to state snapshots |
...
1. `<hermes-root>/skills/ocas-genie/scripts/genie.py` (skill-bundled)
2. `<hermes-root>/scripts/genie.py` (production copy)
3. `<hermes-home>/skills/ocas-genie/scripts/genie.py` (profile skill copy)

Use whichever exists. In production, the script is often at path 2.
**IMPORTANT:** Always use absolute paths (`~/...`), never `~`. In cron context, `~` resolves to the profile-scoped home (`<hermes-home>/home/`), not `~/`.
...
```
Schedule: 0 6 * * 0
Prompt:   Run Genie: python3 <hermes-root>/scripts/genie.py. Report results.
```
...
- **Off-hermes .cache consumers**: When `~/.cache` is large, the usual culprits are: `ms-playwright/` (browser binaries, 1G+ rebuildable), `puccinialin/` (cargo/rustup cache, rebuildable), `chroma/` (embedding DB — check before deleting). These are all Tier 1 safe rebuildable caches. Target them before touching hermes-owned paths.
...
- **`~` path resolution in cron context**: When running as a cron job, `HOME` is set to the profile-scoped path (`<hermes-home>/home/`), not `~/`. The `read_file` tool expands `~` to this profile home, causing "file not found" errors for paths like `~/.hermes/scripts/genie.py`. **Always use absolute paths** (`<hermes-root>/...`) in cron context. The `terminal` tool does not have this issue — it resolves `~` from the actual user home.
